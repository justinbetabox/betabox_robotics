...
# --no-deps prevents pip from replacing Debian's system NumPy, which is
...
"$JUPYTERHUB_VENV_DIR/bin/python" \
    -m pip install \
    --upgrade \
    pip \
    setuptools \
...
"$JUPYTERHUB_VENV_DIR/bin/python" \
    -m pip install \
    -r "$SDK_DIR/deployment/jupyterhub/requirements.txt"

echo "Installing JupyterHub configuration..."

render_service_template \
    "$SDK_DIR/deployment/jupyterhub/jupyterhub_config.py" \
    "$JUPYTERHUB_DIR/jupyterhub_config.py"

if [[ -d "$SDK_DIR/deployment/jupyterhub/theme" ]]; then
    echo "Installing JupyterHub theme..."

    rm -rf \
        "$JUPYTERHUB_DIR/theme"

    cp -r \
        "$SDK_DIR/deployment/jupyterhub/theme" \
        "$JUPYTERHUB_DIR/theme"
fi
...
sudo cp \
    "$SDK_DIR/deployment/jupyterhub/theme/static/custom/"* \
    "$JUPYTERHUB_VENV_DIR/share/jupyterhub/static/custom/"
...
"$JUPYTERHUB_VENV_DIR/bin/jupyter" \
    kernelspec remove \
    -f \
    python3 \
    || true
...
for service in "${INSTALLED_SERVICES[@]}"; do
    ...
    if [[ ! -f "$source_service" ]]; then
        echo "ERROR: Missing systemd service:"
        echo "  $source_service"
        exit 1
    fi
...
echo "  betabox urls"
echo
echo "Launchpad and JupyterHub:"
echo "  Run 'betabox urls' after reboot to display the available addresses."
echo
echo "Robot API:"
echo "  Start JupyterHub from Launchpad or open a Python session and use:"
echo "    from betabox_robotics import Robot"
echo "    robot = Robot.default()"
